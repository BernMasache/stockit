...
export function CollectionSchema() {
  return () =>
    yup.object().shape({
      collection: yup.number().required("Collection is required"),
      numberOfDays: yup.number().required("Number of days is required"),
      shareShop: yup.number().required("Share for the shop is required"),
      expenses: yup.number(),
      comment: yup.string(),
      rentalFare: yup.number().required("Monthly rent is required"),
      dateCollected: yup.date().required("Date is required"),
    });
}

export function ShopCollectionSchema() {
  return () =>
    yup.object().shape({
      collection: yup.number().required("Collection is required"),
      //   comment: yup.string(),
...
export function UpdateCollectionSchema() {
  return () =>
    yup.object().shape({
      ...
      collection: yup
        .string()
        .matches(/^[A-Za-z ]+$/, "Invalid input format")
